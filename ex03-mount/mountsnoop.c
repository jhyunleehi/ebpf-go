...
static int probe_exit(void *ctx, int ret) {
  ...
  if (argp->src)
  	bpf_probe_read_user_str(eventp->src, sizeof(eventp->src), (const void *) argp->src);
  else
  	eventp->src[0] = '\0';
  if (argp->dest)
  	bpf_probe_read_user_str(eventp->dest, sizeof(eventp->dest), (const void *) argp->dest);
  else
  	eventp->dest[0] = '\0';
  if (argp->fs)
  	bpf_probe_read_user_str(eventp->fs, sizeof(eventp->fs), (const void *) argp->fs);
  else
  	eventp->fs[0] = '\0';
  if (argp->data)
